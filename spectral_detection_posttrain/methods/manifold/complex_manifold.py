...
class ComplexMLP(nn.Module):
    ...
    Args:
        ...
        near_identity: if ``True`` and ``identity_init`` is set, add a small
            perturbation after the identity initialization so that gradients
            can flow at the very beginning of training.
        near_identity_noise: standard deviation of the perturbation.  The
            effective scale is ``noise / sqrt(max_dim)`` to keep the output
    ...
    def __init__(
        ...
        near_identity_noise: float = 1e-3,
    ):
        super().__init__()
        self.in_features = in_features
        self.out_features = out_features
        ...
        if identity_init:
            self._init_identity()
        if identity_init and near_identity:
            self._perturb_identity(near_identity_noise)
    ...
    def _perturb_identity(self, noise: float = 1e-3) -> None:
        r"""Add a small perturbation around the identity initialization.

        Without this perturbation, if the module is combined with a delta
        residual ``out = x + alpha * (manifold(F) - F)``, the initial delta is
        exactly zero and no gradient reaches the manifold weights.  A small
        near-identity perturbation breaks the dead lock while keeping the
        module close to identity at start.
        """
        max_dim = max(self.in_features, self.hidden_features, self.out_features)
        std = noise / math.sqrt(max_dim)
        with torch.no_grad():
            self.fc1.weight += torch.randn_like(self.fc1.weight) * std
            self.fc2.weight += torch.randn_like(self.fc2.weight) * std
            if self.fc1.bias is not None:
                self.fc1.bias += torch.randn_like(self.fc1.bias) * std
            if self.fc2.bias is not None:
                self.fc2.bias += torch.randn_like(self.fc2.bias) * std
            # A small negative activation bias prevents strict identity
            # through the modReLU as well.
            self.activation.bias -= noise * 0.1
