"""Plan 2.112: DPO with amp_lo manifold distance-based pair selection.
...
For each GT with >=2 matching proposals:
  ...
  - Closest = chosen, farthest = rejected

DPO loss: same as round2108 (pairwise best-vs-worst contrastive on logit margins).
"""
...
def run_one(cfg_name, mode, seed, scaler, pca, tp_median):
    run_name = f"round2112_{cfg_name}_s{seed}"; set_seed(seed)
    model = bm().to(DEV)
