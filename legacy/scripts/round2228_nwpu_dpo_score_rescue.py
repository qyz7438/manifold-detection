"""Round 2.228: NWPU DPO score-rescue policy (Phase 1 continuation).

Instead of GRPO sample-based reward, use Direct Preference Optimization on
pairs of low-confidence candidates:

  preferred:  low-conf box with IoU > iou_pos_min
  dispreferred: low-conf box with IoU < iou_neg_max

The policy learns to assign a higher rescued score to the preferred box than to
the dispreferred box.  This directly targets the ranking that NMS/eval care
about, avoiding the sparse reward problem of GRPO.
"""
...
from spectral_detection_posttrain.methods.rlvr.roi_policy_loss import resize_boxes_to_image
...
class ScoreAdjustmentPolicy(nn.Module):
    """Deterministic policy: outputs a single score delta."""

    def __init__(self, in_features: int, hidden_dim: int = 128, max_delta: float = 0.3):
        super().__init__()
    ...
    def forward(self, box_features: torch.Tensor):
        return torch.tanh(self.head(self.net(box_features)).squeeze(-1)) * self.max_delta


def build_policy(model, hidden_dim: int = 128, max_delta: float = 0.3) -> ScoreAdjustmentPolicy:
    in_features = model.roi_heads.box_predictor.cls_score.in_features
    return ScoreAdjustmentPolicy(in_features, hidden_dim, max_delta).to(next(model.parameters()).device)


def build_dpo_pairs(proposals, cfg: dict):
    """Return (features_pos, scores_pos, features_neg, scores_neg) tensors."""
    device = proposals["box_features"].device
    scores = proposals["scores"].to(device)
    ious = proposals["ious"].to(device)
    features = proposals["box_features"]

    low_conf = scores < float(cfg["low_conf_max"])
    pos_mask = low_conf & (ious >= float(cfg["iou_pos_min"]))
    neg_mask = low_conf & (ious <= float(cfg["iou_neg_max"]))
    ...
    # Cartesian product of positives and negatives within this image
    n_pos = features_pos.shape[0]
    n_neg = features_neg.shape[0]
    features_pos_rep = features_pos.unsqueeze(1).expand(n_pos, n_neg, -1).reshape(n_pos * n_neg, -1)
    scores_pos_rep = scores_pos.unsqueeze(1).expand(n_pos, n_neg).reshape(-1)
    features_neg_rep = features_neg.unsqueeze(0).expand(n_pos, n_neg, -1).reshape(n_pos * n_neg, -1)
    scores_neg_rep = scores_neg.unsqueeze(0).expand(n_pos, n_neg).reshape(-1)

    return features_pos_rep, scores_pos_rep, features_neg_rep, scores_neg_rep


def dpo_loss(policy, ref_policy, features_pos, scores_pos, features_neg, scores_neg, beta: float):
    delta_pos = policy.forward(features_pos)
    delta_neg = policy.forward(features_neg)
    s_pos = scores_pos + delta_pos
    s_neg = scores_neg + delta_neg

    with torch.no_grad():
        ref_delta_pos = ref_policy.forward(features_pos)
        ref_delta_neg = ref_policy.forward(features_neg)
    ref_s_pos = scores_pos + ref_delta_pos
    ref_s_neg = scores_neg + ref_delta_neg

    logits = beta * ((s_pos - s_neg) - (ref_s_pos - ref_s_neg))
...
def evaluate_policy(model, policy, val_loader, device, cfg: dict):
    ...
    for images, batch_targets in val_loader:
        ...
        if low_conf_mask.any():
            delta[low_conf_mask] = policy.forward(props["box_features"][low_conf_mask])
...
def train(model, policy, ref_policy, train_loader, val_loader, device, cfg: dict, run_dir: Path):
    ...
    for epoch in range(int(cfg["epochs"])):
        ...
        for images, batch_targets in pbar:
            ...
            pairs = build_dpo_pairs(proposals, cfg)
            if pairs is None:
                continue

            loss, acc = dpo_loss(policy, ref_policy, *pairs, beta=float(cfg["beta"]))
            ...
            n_pairs = pairs[0].shape[0]
            epoch_loss += float(loss.item()) * n_pairs
            epoch_acc += acc * n_pairs
            epoch_pairs += n_pairs
            pbar.set_postfix({"loss": f"{loss.item():.4f}", "acc": f"{acc:.2f}"})

        avg_loss = epoch_loss / max(1, epoch_pairs)
        avg_acc = epoch_acc / max(1, epoch_pairs)

        val_metrics = evaluate_policy(model, policy, val_loader, device, cfg)
        record = {
            "epoch": epoch + 1,
            "train_loss": avg_loss,
            "train_pair_accuracy": avg_acc,
        ...
        print(f"Epoch {epoch+1}: loss={avg_loss:.4f} pair_acc={avg_acc:.3f} pairs={epoch_pairs} AP50={record['val_ap50']:.4f} AP75={record['val_ap75']:.4f} #pred={record['val_num_predictions']}")
...
def main():
    parser = argparse.ArgumentParser(description="Round 2.228 NWPU DPO score rescue.")
    parser.add_argument("--run-name", default="round2228_nwpu_dpo_score_rescue")
    ...
    parser.add_argument("--iou-pos-min", type=float, default=0.5)
    parser.add_argument("--iou-neg-max", type=float, default=0.3)
    ...
    cfg = {
        ...
        "iou_pos_min": args.iou_pos_min,
        "iou_neg_max": args.iou_neg_max,
    ...
    policy = build_policy(model, hidden_dim=128, max_delta=args.max_delta)
    ref_policy = build_policy(model, hidden_dim=128, max_delta=args.max_delta)
    ref_policy.load_state_dict(policy.state_dict())
    for p in ref_policy.parameters():
        p.requires_grad = False

    baseline_metrics = evaluate_policy(model, policy, val_loader, device, cfg)
    save_json({
    ...
    history, best_epoch, best_ap75 = train(model, policy, ref_policy, train_loader, val_loader, device, cfg, run_dir)
