...
- User-file exclusion: docs/energy_transport_vs_fpn_sm_analysis.md is an untracked user file (may be absent); it is never staged, moved, or treated as a clean-worktree blocker.
...
- rollback command: `git revert <this-commit>`
- remaining risks: Wave B (maintained analysis CLIs -> `scripts/analysis/` with thin old-path wrappers) and Wave C (frozen launchers -> `scripts/archive/historical/` with frozen-status wrappers) pending; each requires its own adjudication of the remaining `proposed` inventory entries. T9 found `det.dpo.smoke.001.json` references a missing runner (scripts/round2129_nwpu_posttrain_smoke.py absent); its supervisor lives at `legacy/scripts/run_round2218_short_dpo_sweep.py` — resolved in T18 by retiring the config to `legacy/configs/versions/`.
...
- files changed: 14 byte-identical moves `scripts/<name>.py` -> `scripts/analysis/<name>.py` (git rename-detected; `git hash-object` verified); 14 thin old-path wrappers (uniform combined template: deprecation note to stderr + importlib load of the moved module with `__file__` set to the old path so `Path(__file__).parents[1]` repo-root resolution in 11 modules keeps working, `globals().update` re-export for importers, `__main__` CLI delegation with exit-code passthrough); new `scripts/analysis/__init__.py` (docstring only); `scripts/archive/manifest.json` 34 -> 48 entries (wave-B fields: `research_status: maintained`, `current_reproduction_wrapper` = old path, `replacement_command` = new invocation, `runnable_state: runnable_via_wrapper`, per-file historical invocation/required artifacts/failure modes); `tests/compatibility/test_script_archive_manifest.py` extended (`DESTINATION_PREFIXES` per wave, wave B/C wrapper branch, new `test_wave_b_wrappers_delegate_to_destination`, wave-A import scan scoped to wave A); regenerated `script_inventory.json` + `docs/research/*.md`; `docs/research/refactor_ledger.md`
- RED command/result: wave-B semantics absent (manifest had 34 wave-A entries, no wrapper-delegation test)
- GREEN command/result: manifest file 7 passed (6 existing + 1 new delegation test); CLI and import spot checks above
- full-suite result: 1451 passed + 1 skipped = 1450+1 baseline + 1 new
- reviewer and findings accepted/rejected: main-orchestrator adjudication and review. Accepted judgment calls: (1) `research_status: maintained` overrides inventory `unknown` for the 14 — evidence is committed importing tests or documented reproduction commands; (2) one uniform wrapper template instead of import/CLI variants — simpler to audit; (3) deferral of `validate_dense_endpoint_cleanval` rather than editing the read-only backfill generator. KNOWN ISSUE surfaced by the builder and confirmed by main: a pre-existing circular import (`endpoint/dense_endpoint.py:11` imports the flat `dense_set_energy` shim, whose re-export chain cycles) makes `tests/test_dense_teacher_component_audit.py::test_summarize_rows_detects_duplicate_support_failure_only` fail in REDUCED test selections (it passes in the full suite due to import order). NOT caused by Wave B (fails identically with HEAD's file restored). Fix scheduled as a separate follow-up commit using the T14c sanctioned import-line precedent (point the import at the canonical sibling `endpoint.dense_set_energy`) plus a subprocess import-order regression test.
...
- adjudication: 11 registry records are `frozen_reproduction_only` with existing `scripts/` entrypoints; 8 eliminated by criterion (b) — their read-only `tests/test_*_script.py` files assert old-path source text or slice function bodies (deferred, same class as Wave B); 3 moved, all verified by main: blob-identical, wrapper exists, correct `historical/<line>/` destination, replacement command is the verified dispatcher dry-run. Main spot check: `python scripts/train_nwpu_dense_endpoint_absolute.py --help` prints the frozen-status note with the dispatcher replacement command, then byte-identical argparse help, exit 0. Deferred (resolved in T18): `det.dpo.smoke.001.json` referenced a missing runner (scripts/round2129_nwpu_posttrain_smoke.py absent; four other scripts import it and would fail at import time) and its supervisor lived at `legacy/scripts/run_round2218_short_dpo_sweep.py`; T18 retired the config to `legacy/configs/versions/det.dpo.smoke.001.json`.
- files changed: 3 byte-identical moves — `scripts/verify_nwpu_native_c1_contract.py` -> `scripts/archive/historical/nwpu_c1/` (record `det.energy.native_listwise.c1.001`), `scripts/train_nwpu_dense_endpoint_absolute.py` and `scripts/train_dense_local_delta_learner.py` -> `scripts/archive/historical/dense/` (records `det.energy.dense_endpoint.absolute.001`, `det.energy.dense_local_delta_learner.001`); 3 thin old-path wrappers (combined template: frozen-status note to stderr naming the dispatcher replacement + importlib re-export with `__file__` preservation so the locked configs' `Path(__file__).parents[1]` root resolution and internal sha256 checks keep working + `__main__` CLI delegation with identical argv/exit code); `scripts/archive/manifest.json` 48 -> 51 entries (wave-C: `research_status: frozen_reproduction`, `replacement_command` = dispatcher dry-run with canonical run names from the `.sh` launchers, `runnable_state: runnable_via_wrapper`); `tests/compatibility/test_script_archive_manifest.py` extended (`test_wave_c_wrappers_delegate_and_announce_frozen_status`); regenerated `script_inventory.json` + `docs/research/*.md`; `docs/research/refactor_ledger.md`
- RED command/result: wave-C semantics absent (no historical destinations, no frozen-status wrapper test)
- GREEN command/result: manifest file 8 passed; collateral set (3 moved-script tests + 8 deferred-launcher source-scan tests + dispatcher/registry/research-status + backfill + inventory) 216 passed; dispatcher dry-run verified for all 3 records post-move (rendered `python <legacy_entrypoint>` commands stay valid via wrappers)
- full-suite result: 1453 passed + 1 skipped = 1452+1 baseline + 1 new
- reviewer and findings accepted/rejected: main-orchestrator adjudication and review. Accepted judgment calls: (1) line dirs `nwpu_c1`/`dense` at program level (the two dense scripts share `dense/` despite distinct registry research_lines); (2) `kind` kept from inventory rather than relabeled "launcher"; (3) combined wrapper template matching Wave B. KNOWN ISSUE surfaced by the builder and confirmed by main: a SECOND pre-existing circular import (`set_policy` <-> `policy.global_top1` through flat shims, sibling of the e181501 fix) makes `python scripts/verify_nwpu_native_c1_contract.py --help` fail identically at HEAD in a fresh process — no regression from the move; the script's tests pass because pytest collection order initializes the package first. Fix scheduled as a follow-up commit using the same sanctioned import-line precedent (repoint the two intra-policy sibling imports to canonical `policy.*` paths) plus a subprocess regression test.
...
- RED command/result: `python scripts/dev/check_docs.py --json` -> 8 errors (broken links to current_status.md and archive/README.md; broken paths to untracked user file, stale scripts, missing archive dirs, missing verifiers.py source)
- GREEN command/result: `python scripts/dev/check_docs.py --json` -> 0 errors, 372 warnings (all in historical plan/docs/reports that are out of refactor scope); `pytest tests/contracts/test_documentation_integrity.py tests/experiments/test_generated_research_docs.py -q` -> 49 passed, 1 skipped
- full-suite result: 1500 passed, 3 skipped, 2 xfailed (run with `E:/anaconda/01/envs/RLimage/python.exe -m pytest tests -q`)
- reviewer and findings accepted/rejected: main-orchestrator self-review. Accepted: (1) warnings are tolerated for historical docs; only errors block the contract; (2) DPO smoke config retired to `legacy/configs/versions/` because its runner is absent and its supervisor lives in `legacy/scripts/`; (3) `docs/reports/archive/` created with a README to satisfy the active report index links; (4) `models/verifiers.py` migration recorded as completed because both canonical and historical destinations exist and the old path does not.
