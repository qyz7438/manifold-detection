"""Plan 2.64 v2: IoU + ROI + shuffled FFT verifier GRPO, per-sampled-box. CONTROL."""
...
def shuffle_fft_feats(fft):
    idx=torch.randperm(fft.shape[0],device=fft.device)
    return fft[idx]
...
def main():
    all_r=[]
    for lbd in [0.1,0.5]:
        run_name=f"round264_shuf14_l{lbd}_s42";set_seed(42)
            ...
            for imgs,tgts in tqdm(tl,desc=f"{run_name} e{ep}"):
                ...
                if rf is not None and pr is not None and rf.shape[0]>0 and fpn is not None:
                    ...
                    for ki,k in enumerate(fpn_keys):
                        ...
                        fft_shuf=shuffle_fft_feats(fft_f)
                        if vrf is None:
                            vrf=FFTVerifier(r.flatten(1).shape[1],fft_shuf.shape[1]).to(DEV)
    ...
    print("\n## Plan 2.64 v2 Per-Box Shuffled FFT Verifier Results")
