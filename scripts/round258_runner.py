"""Plan 2.58: DPO with Edge Truncation Quality.

Key insight: a well-placed box has NO edges at its boundaries (object is fully inside).
A shifted box that cuts the object has many edges crossing the boundary strip.
quality = 1 - edges_in_boundary / total_edges.

This is delta-dependent (pixel patches from decoded boxes), fully physics-based
(no FFT, no CNN, no GT), and naturally sensitive to 1-2px shifts.
"""
...
EPOCHS = 10
PIXEL_SIZE = 64
BOUNDARY_WIDTH = 3  # px from each edge to check for truncation
QUALITY_MARGIN = 0.02


def edge_truncation_quality(patches):
    """Sobel edges → boundary strip energy → quality = 1 - boundary/total.

    High quality = edges are NOT at boundaries = box is well-placed.
    Low quality = edges concentrated at boundaries = box cuts through object.
    """
    gray = patches.float().mean(dim=1)  # (N, 64, 64)
    gx = gray[:, :, 1:] - gray[:, :, :-1]  # (N, 64, 63)
    gy = gray[:, 1:, :] - gray[:, :-1, :]  # (N, 63, 64)
    edge = torch.sqrt(gx[:, :-1, :].pow(2) + gy[:, :, :-1].pow(2) + 1e-6)  # (N, 63, 63)
    total_edge = edge.flatten(1).sum(dim=1).clamp_min(1e-6)  # (N,)

    # Boundary mask: pixels within BOUNDARY_WIDTH of edges
    H, W = 63, 63
    bw = BOUNDARY_WIDTH
    boundary_mask = torch.zeros(H, W, device=patches.device, dtype=torch.bool)
    boundary_mask[:bw, :] = True   # top
    boundary_mask[-bw:, :] = True  # bottom
    boundary_mask[:, :bw] = True   # left
    boundary_mask[:, -bw:] = True  # right

    boundary_edge = (edge * boundary_mask.float().unsqueeze(0)).flatten(1).sum(dim=1)  # (N,)

    # Quality: 1 - boundary fraction. High = good.
    quality = 1.0 - boundary_edge / total_edge
...
def main():
    ...
    for beta in BETAS:
        run_name = f"round258_trunc_b{beta}_s42"
        set_seed(42)
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            for images, targets in tqdm(train_loader, desc=f"{run_name} e{epoch}"):
                ...
                if roi_feats is not None and proposals is not None and roi_feats.shape[0] > 0:
                    ...
                    # Decode + pixel patches + edge truncation quality
                    ...
                    Kt = N * K_SAMPLES
                    evaluated = torch.zeros(Kt, dtype=torch.bool, device=DEV)
                    if patches:
                        pb = torch.stack(patches).to(DEV)
                        q_all = edge_truncation_quality(pb)
                        qp = torch.zeros(Kt, device=DEV); qp[:len(q_all)] = q_all
                        evaluated[:len(q_all)] = True
                        q_matrix = qp.view(N, K_SAMPLES)
                    else:
                        q_matrix = torch.zeros(N, K_SAMPLES, device=DEV)

                    evaluated_matrix = evaluated.view(N, K_SAMPLES)
                    q_diff = q_matrix[:, 0] - q_matrix[:, 1]
                    chosen = q_diff >= 0
                    valid = evaluated_matrix.all(dim=1) & (q_diff.abs() > QUALITY_MARGIN)
    ...
    print("\n## Plan 2.58 Edge Truncation DPO Results")
    for r in all_r:
