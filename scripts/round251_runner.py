"""Plan 2.51 v2: Combined Spectral DPO — all 3 channels from pixel patches.

Fixes v1 bug: radial_energy_quality now uses pixel patch FFT, making all 3
metrics truly delta-dependent and orthogonal.
"""
...
def radial_energy_quality(patches):
    """Pixel patch -> FFT -> 3 radial rings -> 0.5*low + 0.3*high."""
    gray = patches.float().mean(dim=1)
    fft = torch.fft.fft2(gray).abs()
    mag_pow = fft.pow(2)
    H, W = mag_pow.shape[-2], mag_pow.shape[-1]
    ...
    low_e = mag_pow[:, r_n <= 0.15].sum(dim=1)
    mid_e = mag_pow[:, (r_n > 0.15) & (r_n <= 0.4)].sum(dim=1)
    high_e = mag_pow[:, r_n > 0.4].sum(dim=1)
    total = (low_e + mid_e + high_e).clamp_min(1e-6)
    return (0.5 * low_e / total + 0.3 * high_e / total).clamp(0, 1)
...
def edge_boundary_quality(patches):
    ...
    b = 8; ring = torch.zeros_like(edge)
    ring[:, :b, :] = 1; ring[:, -b:, :] = 1; ring[:, :, :b] = 1; ring[:, :, -b:] = 1
    return ((edge * ring).flatten(1).sum(dim=1) / total).clamp(0, 1)
...
def main():
    ...
    for beta in BETAS:
        run_name = f"round251_v2_b{beta}_s42"
        set_seed(42)
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            for images, targets in tqdm(train_loader, desc=f"{run_name} e{epoch}"):
                ...
                if roi_feats is not None and proposals is not None and roi_feats.shape[0] > 0:
                    ...
                    # Decode and crop pixel patches (ALL 3 channels use these)
                    ...
                    Kt = N * K_SAMPLES
                    q_radial = torch.zeros(N, K_SAMPLES, device=DEV)
                    q_edge = torch.zeros(N, K_SAMPLES, device=DEV)

                    if patches_list:
                        pb = torch.stack(patches_list).to(DEV)
                        # Channel 1: Radial energy from pixel FFT (delta-dependent!)
                        qr = radial_energy_quality(pb)
                        qrp = torch.zeros(Kt, device=DEV); qrp[:len(qr)] = qr
                        q_radial = qrp.view(N, K_SAMPLES)
                        # Channel 2: Edge boundary ring energy
                        qe = edge_boundary_quality(pb)
                        qep = torch.zeros(Kt, device=DEV); qep[:len(qe)] = qe
                        q_edge = qep.view(N, K_SAMPLES)

                    # Channel 3: IoU overlap
                    q_overlap = torch.zeros(N, K_SAMPLES, device=DEV)
                    pim = []; nb = 0
                    ...
                    for pi in range(N):
                        ...
                        if len(gt_boxes) > 0:
                            ...
                            q_overlap[pi] = ious.max(dim=1).values

                    # Pareto preference: 2/3 wins
                    wins_0 = (q_radial[:, 0] > q_radial[:, 1]).float() + (q_edge[:, 0] > q_edge[:, 1]).float() + (q_overlap[:, 0] > q_overlap[:, 1]).float()
                    wins_1 = 3.0 - wins_0
                    chosen = wins_0 >= 2
                    rejected = wins_1 >= 2
                    valid = chosen | rejected
    ...
    print("\n## Plan 2.51 v2 Combined Spectral DPO Results")
    for r in all_r:
