"""Plan 2.83: C-gated Energy Residual v2 — fixes two bugs from 2.82.

Fixes (GPT-5.5 audit):
  1. C-gate now per-group (max IoU), not per-sample — matches counterfactual evidence
  2. BETA applied AFTER GRPO (adv = GRPO(R_loc) + BETA * gated), not before
     — prevents GRPO normalization from amplifying energy to full strength

Formulation: adv = GRPO(R_loc) + 0.02 * clip(-z_energy, -1, 1) * 1[group_max_IoU in 0.35-0.55]
"""
...
def run_one(cfg_name, mode, seed):
    run_name = f"round283_{cfg_name}_s{seed}"
    set_seed(seed)
    model = bm().to(DEV)
...
if __name__ == "__main__":
    ...
    print("\n## Plan 2.83 C-gated Energy Residual v2 (per-group gate + BETA after GRPO)")
