"""Plan 2.67: Per-channel FFT verifier (14×14, NO mean over channels).

Diff from 2.65: FFT extracted per-channel with 3-band radial energy, not mean over 256ch.
Total: 256ch × 3 bands × 2 (amp+phase) = 1536 dims vs 224 for 2.65.
"""
...
class PerChannelFFTVerifier(nn.Module):
    """Per-channel FFT features (no channel mean). 256ch×3bands×2 = 1536."""
...
def extract_perchan_fft(roi):
    """Per-channel rFFT → 3-band radial energy → per-channel compact features."""
    C=roi.shape[1];H,W=roi.shape[-2],roi.shape[-1]
    fft=torch.fft.rfft2(roi,dim=(-2,-1),norm="ortho")  # (N,C,H,W/2+1)
    amp=torch.abs(fft)  # (N,C,H,rW)
    pha=torch.angle(fft)
    freq=torch.fft.fftfreq(max(H,W),device=roi.device)
    Y,X=torch.meshgrid(freq[:H],freq[:W//2+1],indexing='ij')
    r=torch.sqrt(X**2+Y**2)
    r_n=r/r.max().clamp_min(1e-6)
    lo=(r_n<=0.15).float().unsqueeze(0).unsqueeze(0)  # 3 bands
    md=((r_n>0.15)&(r_n<=0.4)).float().unsqueeze(0).unsqueeze(0)
    hi=(r_n>0.4).float().unsqueeze(0).unsqueeze(0)
    a_lo=(amp*lo).flatten(2).sum(dim=2);a_md=(amp*md).flatten(2).sum(dim=2);a_hi=(amp*hi).flatten(2).sum(dim=2)
    p_lo=(pha*lo).flatten(2).sum(dim=2);p_md=(pha*md).flatten(2).sum(dim=2);p_hi=(pha*hi).flatten(2).sum(dim=2)
    return torch.cat([a_lo,a_md,a_hi,p_lo,p_md,p_hi],dim=1)  # (N, 6*C)
...
def main():
    all_r=[]
    for lbd in [0.1,0.5]:
        run_name=f"round267_perchan_l{lbd}_s42";set_seed(42)
        ...
        diag={"q_ious":[],"reward_mean":[],"q_std":[]}  # per-epoch diagnostics
        for ep in range(1,EPOCHS+1):
            ...
            for imgs,tgts in tqdm(tl,desc=f"{run_name} e{ep}"):
                ...
                if rf is not None and pr is not None and rf.shape[0]>0 and fpn is not None:
                    ...
                    for ki,k in enumerate(fpn_keys):
                        ...
                        if vrf is None:
                            vrf=PerChannelFFTVerifier(fft_f.shape[1]).to(DEV)
                    ...
                    diag["reward_mean"].append(iou_r.mean().item())
            ...
            q_s=np.mean(diag["q_std"]);r_m=np.mean(diag["reward_mean"])
            row={"epoch":ep,"val_ap50":em["ap50"],"val_ap75":em["ap75"],"q_iou_corr":float(q_corr),"q_std":float(q_s),"reward_mean":float(r_m)};h.append(row)
            print(f"  e{ep}: AP50={em['ap50']:.4f} det={td:.1f} rl={trl:.3f} v={tv:.3f} pos={pos} q_corr={q_corr:.4f}")
            if em["ap50"]>best:best=em["ap50"]
            diag["q_ious"].clear();diag["q_std"].clear();diag["reward_mean"].clear()
    ...
    print("\n## Plan 2.67 v7 Per-Channel FFT Results")
