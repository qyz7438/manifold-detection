"""Plan 2.62 v2: IoU + ROI verifier GRPO, per-sampled-box quality.

FIXES:
  1. q_pred per sampled box (ROI Align on each decoded box), not per proposal
  2. q_pred NOT detached in verifier loss (verifier actually trains)
  3. Reward = IoU + lbd * q_pred with per-box advantage
"""
...
DEV="cuda";CKPT="runs/round227_v1_baseline_20ep/checkpoint_best.pth";G=4;EPOCHS=5;ROI_SIZE=14  # 14x14 spatial features

class PerBoxROIVerifier(nn.Module):
    """Verifier on per-sampled-box ROI features."""
    def __init__(self,in_dim=256*7*7,hidden=128):
        super().__init__()
        self.net=nn.Sequential(nn.Linear(in_dim,hidden),nn.ReLU(),nn.Linear(hidden,32),nn.ReLU(),nn.Linear(32,1),nn.Sigmoid())
...
def main():
    all_r=[]
    for lbd in [0.1,0.5]:
        run_name=f"round262_roi14_l{lbd}_s42";set_seed(42)
        model=bm().to(DEV);ckpt=torch.load(CKPT,map_location=DEV);model.load_state_dict(ckpt["model"])
        vrf=None
        ref=copy.deepcopy(model);fe(ref,[]);ref.eval()
            ...
            for imgs,tgts in tqdm(tl,desc=f"{run_name} e{ep}"):
                ...
                if rf is not None and pr is not None and rf.shape[0]>0 and fpn is not None:
                    ...
                    for ki,k in enumerate(fpn_keys):
                        ...
                        sc=1.0/(2**(ki_lvl+2))
                        rois=roi_boxes[mask]
                        r=torchvision.ops.roi_align(fpn[k],rois,output_size=ROI_SIZE,spatial_scale=sc)
                        if vrf is None:
                            vrf=PerBoxROIVerifier(r.flatten(1).shape[1]).to(DEV)
                    ...
                    # RL reward: per-box quality (not constant across samples!)
                    reward=iou_r+lbd*q_pred.detach()
                    # Global EMA baseline — prevents λq cancellation in group norm
                    # Global EMA baseline prevents λq group cancellation
    ...
    print("\n## Plan 2.62 v2 Per-Box ROI Verifier Results")
