"""Plan 2.102: Aggressive RLVR — test NMS bottleneck hypothesis.

RL_WEIGHT=0.5 (5x round301), KL_WEIGHT=0.01 (3x weaker anchor).
If RLVR confidence shifts cross NMS ordering thresholds, AP should jump.
"""
import copy
import shutil
import subprocess
...
def run_one(cfg_name, mode, seed):
    run_name = f"round2102_{cfg_name}_s{seed}"
    set_seed(seed)
    model = bm().to(DEV)
...
if __name__ == "__main__":
    ...
    print("\n## Plan 2.102 RLVR on Classifier — IoU-Calibrated Confidence Reward")
