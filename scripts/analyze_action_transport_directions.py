"""Compatibility wrapper: implementation moved to ``scripts/analysis/analyze_action_transport_directions.py``.
...
- importing ``scripts.analyze_action_transport_directions`` re-exports the moved module's public names;
- running ``python scripts/analyze_action_transport_directions.py ...`` delegates to the moved module with
  the same arguments, preserving its historical ``__file__`` (the moved module
  resolves repository paths from it).

Update call sites to ``scripts/analysis/analyze_action_transport_directions.py``; this wrapper will be
...
_DESTINATION = _OLD_PATH.parent / "analysis" / "analyze_action_transport_directions.py"
...
if __name__ == "__main__":
    print(
        "deprecated: scripts/analyze_action_transport_directions.py moved to scripts/analysis/analyze_action_transport_directions.py; "
        "delegating with identical arguments",
        file=_sys.stderr,
    )
    _load_moved_module("__main__")
else:
    _moved = _load_moved_module("scripts.analysis.analyze_action_transport_directions")
    globals().update(
