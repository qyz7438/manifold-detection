"""Final same-cache C3 smoke using balanced unified-score margins."""
...
CONFIG = ROOT / "spectral_detection_posttrain" / "configs" / "versions" / "det.energy.global_delta_u.c3b.balanced.001.json"
CONFIG_SHA256 = "c859e31f42faccbb0c87d0807c03dce0b79477389ff85c913363c27f5e7195d0"
C3_SCRIPT = ROOT / "scripts" / "train_nwpu_c3_global_delta_u.py"


def _load_c3():
    spec = importlib.util.spec_from_file_location("c3b_source", C3_SCRIPT)
...
def load_config() -> dict[str, Any]:
    if c3.m1.sha256_file(CONFIG) != CONFIG_SHA256:
        raise ValueError("canonical C3b config SHA256 mismatch")
    config = json.loads(CONFIG.read_text(encoding="utf-8"))
    if config.get("version_id") != "det.energy.global_delta_u.c3b.balanced.001":
        raise ValueError("C3b version mismatch")
    if config["policy_override"].get("loss_type") != "balanced_margin":
        raise ValueError("C3b balanced loss drift")
    return config


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--run-dir", type=Path, default=ROOT / "runs" / "nwpu_c3b_balanced_smoke_s42")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if c3.CONFIG_SHA256 != config["source"]["base_config_sha256"]:
        raise RuntimeError("C3 base config hash mismatch")
    source_result = ROOT / config["source"]["result"]
    source_cache = ROOT / config["source"]["cache"]
    if c3.m1.sha256_file(source_result) != config["source"]["result_sha256"]:
        raise RuntimeError("C3 source result hash mismatch")
    if c3.m1.sha256_file(source_cache) != config["source"]["cache_sha256"]:
        raise RuntimeError("C3 global_delta_u_cache.pt hash mismatch")
    ...
    if payload.get("format") != "c3_global_delta_u_v1" or not isinstance(payload.get("records"), list):
        raise RuntimeError("locked C3 cache format mismatch")
    if payload.get("metadata", {}).get("config_sha256") != config["source"]["base_config_sha256"]:
        raise RuntimeError("locked C3 cache provenance mismatch")
    ...
    result = {
        "completed": True,
        "scientific_status": "smoke_signal_detected" if all_passed else "current_route_frozen",
        "version_id": config["version_id"],
        "experiment_scope": "final_same_cache_smoke_32_32",
