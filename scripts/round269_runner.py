"""Plan 2.69: Per-channel FFT SHUFFLED control. Same as 2.67 but FFT features permuted across batch."""
...
def fe(m,parts):
    ...
    for part in parts:
        if isinstance(part,nn.Module):[setattr(p,'requires_grad',True) for p in part.parameters()]
        else:
...
def main():
    all_r=[]
    for lbd in [0.1,0.5]:
        run_name=f"round269_shuf14_l{lbd}_s42";set_seed(42)
        ...
        for ep in range(1,EPOCHS+1):
            ...
            for imgs,tgts in tqdm(tl,desc=f"{run_name} e{ep}"):
                ...
                if rf is not None and pr is not None and rf.shape[0]>0 and fpn is not None:
                    ...
                    for ki,k in enumerate(fpn_keys):
                        ...
                        fft_shuf=fft_f[torch.randperm(fft_f.shape[0],device=DEV)]
                        if vrf is None:
                            vrf=PerChanFFT(fft_shuf.shape[1]).to(DEV)
                        ...
                        q_pred[mask]=vrf(fft_shuf)
                    q_pred=q_pred.view(N,G)
                    diag["q_ious"].extend(list(zip(q_pred.flatten().tolist(),iou_r.flatten().tolist())))
                    diag["q_std"].append(q_pred.std().item());diag["reward_mean"].append(iou_r.mean().item())
                    ...
                    adv=reward-baseline[0];pm=iou_r.max(dim=1).values>0.3
                    if pm.any():rl=-(adv[pm].detach()*log_probs[pm]).mean();pos+=pm.sum().item()
                if vrf is None:loss=det
                else:loss=det+vloss+0.01*rl;opt.zero_grad(set_to_none=True);loss.backward();opt.step()
            ...
            h.append({"epoch":ep,"val_ap50":em["ap50"],"val_ap75":em["ap75"],"q_iou_corr":float(q_corr)})
            print(f"  e{ep}: AP50={em['ap50']:.4f} det={td:.1f} rl={trl:.3f} v={tv:.3f} q_corr={q_corr:.4f}")
    ...
    print("\n## Plan 2.69 Per-Chan Shuffled FFT Results")
