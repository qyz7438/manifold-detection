"""NWPU VHR-10: fine-tune box_head + AFM, freeze backbone+RPN.

Test whether the box head needs to co-adapt with the inserted AFM.
"""
import sys, json
...
for seed in SEEDS:
    ...
    # Freeze backbone + RPN; train box_head + AFM
    ...
    # classifier + regressor are inside box_head via AFMThenHead wrapper? Actually original_box_head includes
    # the two FastRCNN heads; yes.
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        for imgs, tgts in tqdm(train_loader, desc=f"boxhead_afm s{seed} e{ep}"):
        ...
        if em['ap50'] > best_ap50:
            ...
            torch.save({"model": model.state_dict(), "ap50": best_ap50, "ap75": em['ap75']},
                       f"runs/nwpu_boxhead_afm_s{seed}_best.pth")
