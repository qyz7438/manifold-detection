"""Plan 2.70: Per-channel FFT BAND-PERMUTED control. Permute within radial bands."""
...
def band_permute(fft_f,C=256):
    ...
    for b in range(6):
        ...
        out[:,sl]=fft_f[torch.randperm(B,device=fft_f.device)][:,sl]
    return out
...
def main():
    all_r=[]
    for lbd in [0.1,0.5]:
        run_name=f"round270_band14_l{lbd}_s42";set_seed(42)
        ...
        for ep in range(1,EPOCHS+1):
            ...
            for imgs,tgts in tqdm(tl,desc=f"{run_name} e{ep}"):
                ...
                if rf is not None and pr is not None and rf.shape[0]>0 and fpn is not None:
                    ...
                    for ki,k in enumerate(fpn_keys):
                        ...
                        fft_bf=band_permute(extract_perchan_fft(r))
                        if vrf is None:
                            vrf=PerChanFFT(fft_bf.shape[1]).to(DEV)
                        ...
                        q_pred[mask]=vrf(fft_bf)
                    q_pred=q_pred.view(N,G);vloss=F.mse_loss(q_pred,iou_r.detach())
            ...
            em=ev(model,vl);h.append({"epoch":ep,"val_ap50":em["ap50"],"val_ap75":em["ap75"]})
            print(f"  e{ep}: AP50={em['ap50']:.4f} det={td:.1f} rl={trl:.3f} v={tv:.3f}")
    ...
    print("\n## Plan 2.70 Band-Permuted FFT Results")
