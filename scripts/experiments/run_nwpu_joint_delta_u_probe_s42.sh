...
' "$RESULT" "$CURRENT_HEAD" "$CONFIG_SHA"; then
  printf 'joint Delta-U probe already completed for commit %s and locked config\n' "$CURRENT_HEAD"
  exit 0
fi
...
exec "$PYTHON" scripts/probe_nwpu_joint_delta_u.py \
  --config spectral_detection_posttrain/configs/versions/det.energy.joint_probe.001.json \
  --run-dir "$RUN_DIR" \
  --device cuda \
  --require-clean-git \
  --require-full-train
