...
wait_for_4ep_control() {
  if [[ "${WAIT_FOR_4EP}" != "1" ]]; then
    return 0
  fi

  local s42="runs/ctrl_boxhead_ft_fullnw0_nwpu_s42_bs${BATCH_SIZE}_ep4/eval_metrics.json"
  local s2024="runs/ctrl_boxhead_ft_fullnw0_nwpu_s2024_bs${BATCH_SIZE}_ep4/eval_metrics.json"
  while [[ ! -f "${s42}" || ! -f "${s2024}" ]]; do
    echo "==== $(date -Is) waiting for 4ep box-head control outputs ===="
    echo "s42=$([[ -f "${s42}" ]] && echo done || echo pending)"
    echo "s2024=$([[ -f "${s2024}" ]] && echo done || echo pending)"
    sleep 300
  done
}
...
run_one() {
  ...
  local run_name="ctrl_boxhead_ft10_fullnw0_nwpu_s${seed}_bs${BATCH_SIZE}_ep${EPOCHS}"
...
echo "==== $(date -Is) box-head 10ep fine-tune control complete ===="
