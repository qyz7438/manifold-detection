...
RUN_DIR="$ROOT/runs/nwpu_dense_local_delta_stats_s42_train64"
...
if [[ -f "$RUN_DIR/eval_metrics.json" ]]; then
  echo "dense local Delta-Q stats already completed"
  exit 0
fi
if pgrep -af "scripts/analyze_dense_local_delta_stats.py" >/dev/null; then
  echo "dense local Delta-Q stats is already running"
  exit 0
fi
...
echo "launching dense local Delta-Q stats: gpu2_free_mib=$GPU2_FREE_MIB estimated_peak_mib=$ESTIMATED_PEAK_MIB"
