...
RUN_DIR="$ROOT/runs/nwpu_c2_native_budget1_smoke_s42"
cd "$ROOT"
...
if [[ -f "$RUN_DIR/eval_metrics.json" ]]; then
  echo "C2 smoke artifact already exists"
  exit 0
fi

export CUDA_VISIBLE_DEVICES=2
mkdir -p "$RUN_DIR"
/home/ps/anaconda3/envs/RLimage/bin/python scripts/train_nwpu_c2_native_budget1.py \
  --run-dir "$RUN_DIR" \
  --limit-train 32 \
  --limit-val 32 \
  --rebuild-cache \
  --require-clean-git 2>&1 | tee "$RUN_DIR/launcher.log"
