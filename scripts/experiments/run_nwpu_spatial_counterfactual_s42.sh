...
RUN_DIR="$ROOT/runs/nwpu_spatial_counterfactual_s42_nested"
PYTHON=/home/ps/anaconda3/envs/RLimage/bin/python
RESULT="$RUN_DIR/eval_metrics.json"
CONFIG_SHA=71e1f3589ee05f7674246b46b24536313574c6fb6c0e087eaad2f19069ce1429
...
' "$RESULT" "$CURRENT_HEAD" "$CONFIG_SHA"; then
  printf 'spatial counterfactual probe already completed for commit %s\n' "$CURRENT_HEAD"
  exit 0
fi
...
exec "$PYTHON" scripts/probe_nwpu_spatial_counterfactual.py \
  --config spectral_detection_posttrain/configs/versions/det.energy.spatial_counterfactual.001.json \
  --run-dir "$RUN_DIR" \
  --device cuda \
  --require-clean-git
