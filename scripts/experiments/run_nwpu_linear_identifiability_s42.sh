...
RUN_DIR="$ROOT/runs/nwpu_linear_identifiability_s42_nested"
PYTHON=/home/ps/anaconda3/envs/RLimage/bin/python
RESULT="$RUN_DIR/eval_metrics.json"
CONFIG_SHA=2de392baf1effd2b9fce7cf9463fef1e844a62b97bce76b8ee0928d49719686a
...
' "$RESULT" "$CURRENT_HEAD" "$CONFIG_SHA"; then
  printf 'linear identifiability probe already completed for commit %s\n' "$CURRENT_HEAD"
  exit 0
fi
...
exec "$PYTHON" scripts/probe_nwpu_linear_identifiability.py \
  --config spectral_detection_posttrain/configs/versions/det.energy.linear_identifiability.001.json \
  --run-dir "$RUN_DIR" \
  --device cuda \
  --require-clean-git
