...
RUN_DIR="$ROOT/runs/nwpu_dense_teacher_stats_s42_innerfit"
...
if [[ -f "$RUN_DIR/eval_metrics.json" ]]; then
  echo "dense teacher stats already completed: $RUN_DIR/eval_metrics.json"
  exit 0
fi
if pgrep -af "scripts/analyze_dense_teacher_components.py.*nwpu_dense_teacher_stats_s42_innerfit" >/dev/null; then
  echo "dense teacher stats is already running"
  exit 0
fi
...
echo "launching dense teacher stats: gpu2_free_mib=$GPU2_FREE_MIB estimated_peak_mib=$ESTIMATED_PEAK_MIB"
exec "$PYTHON" scripts/analyze_dense_teacher_components.py \
  --run-dir "$RUN_DIR" \
  --device cuda \
  --require-clean-git
