...
RUN_DIR="$ROOT/runs/nwpu_c3b_balanced_smoke_s42"
cd "$ROOT"
...
if [[ -f "$RUN_DIR/eval_metrics.json" ]]; then
  echo "C3b artifact already exists"
  exit 0
fi
export CUDA_VISIBLE_DEVICES=2
mkdir -p "$RUN_DIR"
/home/ps/anaconda3/envs/RLimage/bin/python scripts/train_nwpu_c3b_balanced.py \
