...
RUN_DIR="$ROOT/runs/nwpu_c3a_forced_top1_smoke_s42"
cd "$ROOT"
...
if [[ -f "$RUN_DIR/eval_metrics.json" ]]; then
  echo "C3a artifact already exists"
  exit 0
fi
export CUDA_VISIBLE_DEVICES=2
mkdir -p "$RUN_DIR"
/home/ps/anaconda3/envs/RLimage/bin/python scripts/eval_nwpu_c3a_forced_top1.py \
