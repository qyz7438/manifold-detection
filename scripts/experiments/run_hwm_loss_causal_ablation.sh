...
run_one() {
  ...
  local run_name="abl_hwm_loss_${tag}_fullnw0_nwpu_s${seed}_bs${BATCH_SIZE}_ep${EPOCHS}"
...
for seed in ${SEEDS}; do
  ...
  run_one "${seed}" "box_hwm_e0" \
    --energy-weight 0.0 \
    --hwm-weight 0.05 \
    --hwm-epsilon 0.01 \
    --high-iou-preserve-weight 0.0

  run_one "${seed}" "current_e001" \
    --energy-weight 0.01 \
    --hwm-weight 0.05 \
    --hwm-epsilon 0.01 \
    --high-iou-preserve-weight 2.0
...
echo "==== $(date -Is) hwm loss causal ablation complete ===="
