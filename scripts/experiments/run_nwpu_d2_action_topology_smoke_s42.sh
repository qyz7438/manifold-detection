...
RUN_DIR="$ROOT/runs/nwpu_d2_action_topology_smoke_s42"
cd "$ROOT"
...
if [[ -f "$RUN_DIR/eval_metrics.json" ]]; then
  echo "D2 artifact already exists"
  exit 0
fi
export CUDA_VISIBLE_DEVICES=2
mkdir -p "$RUN_DIR"
/home/ps/anaconda3/envs/RLimage/bin/python scripts/train_nwpu_d2_action_topology.py \
