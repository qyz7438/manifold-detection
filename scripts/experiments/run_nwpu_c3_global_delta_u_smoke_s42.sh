...
RUN_DIR="$ROOT/runs/nwpu_c3_global_delta_u_smoke_s42"
cd "$ROOT"
...
if [[ -f "$RUN_DIR/eval_metrics.json" ]]; then
  echo "C3 smoke artifact already exists"
  exit 0
fi
export CUDA_VISIBLE_DEVICES=2
mkdir -p "$RUN_DIR"
/home/ps/anaconda3/envs/RLimage/bin/python scripts/train_nwpu_c3_global_delta_u.py \
  --run-dir "$RUN_DIR" \
  --limit-train 32 \
  --limit-val 32 \
  --rebuild-cache \
