...
EPOCHS="${EPOCHS:-36}"
LR="${LR:-0.005}"
MIN_LR="${MIN_LR:-0.00005}"
PARITY_RUN="det_action_zero_parity_fullft18best_s42"
RUN_NAME="nwpu_mob_strong_cosine_s42_bs8_36ep"
...
if [[ -f "${metrics}" ]]; then
  ...
  echo "Incomplete strong baseline requires review: ${metrics}" >&2
  exit 4
fi
...
echo "$(date -Is) NWPU strong cosine baseline complete"
