...
RUN_DIR="$ROOT/runs/nwpu_dense_endpoint_absolute_s42_nested"
...
if [[ -f "$RUN_DIR/eval_metrics.json" ]]; then
  echo "dense absolute endpoint already completed: $RUN_DIR/eval_metrics.json"
  exit 0
fi
if pgrep -af "scripts/train_nwpu_dense_endpoint_absolute.py.*nwpu_dense_endpoint_absolute_s42_nested" >/dev/null; then
  echo "dense absolute endpoint is already running"
  exit 0
fi
...
echo "launching dense absolute endpoint: gpu2_free_mib=$GPU2_FREE_MIB estimated_peak_mib=$ESTIMATED_PEAK_MIB"
exec "$PYTHON" scripts/train_nwpu_dense_endpoint_absolute.py \
  --run-dir "$RUN_DIR" \
  --device cuda \
  --require-clean-git
