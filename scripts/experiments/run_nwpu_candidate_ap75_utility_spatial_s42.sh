...
RUN_NAME="action_candidate_ap75_utility_spatial_s42_k73_bs8_ep4"
...
wait_for_gpu2() {
  while true; do
    ...
    if [[ "${free_mb}" =~ ^[0-9]+$ ]] && (( free_mb > 8192 )); then
      log "GPU2 free=${free_mb}MB; starting spatial AP75-utility candidate run"
      return
    fi
...
CUDA_VISIBLE_DEVICES=2 "${PYTHON}" scripts/train_candidate_action_energy.py \
  ...
  --gain-target ap75_utility \
  --gain-utility-temperature 0.05 \
  --epochs 4 \
  --batch-size 8 \
  --num-workers 0 \
  --seed 42 \
