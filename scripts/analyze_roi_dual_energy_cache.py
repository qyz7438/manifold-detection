"""Compatibility wrapper: implementation moved to ``scripts/analysis/analyze_roi_dual_energy_cache.py``.
...
- importing ``scripts.analyze_roi_dual_energy_cache`` re-exports the moved module's public names;
- running ``python scripts/analyze_roi_dual_energy_cache.py ...`` delegates to the moved module with
  the same arguments, preserving its historical ``__file__`` (the moved module
  resolves repository paths from it).

Update call sites to ``scripts/analysis/analyze_roi_dual_energy_cache.py``; this wrapper will be
...
_DESTINATION = _OLD_PATH.parent / "analysis" / "analyze_roi_dual_energy_cache.py"
...
if __name__ == "__main__":
    print(
        "deprecated: scripts/analyze_roi_dual_energy_cache.py moved to scripts/analysis/analyze_roi_dual_energy_cache.py; "
        "delegating with identical arguments",
        file=_sys.stderr,
    )
    _load_moved_module("__main__")
else:
    _moved = _load_moved_module("scripts.analysis.analyze_roi_dual_energy_cache")
    globals().update(
