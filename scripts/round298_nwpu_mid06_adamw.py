"""NWPU VHR-10: mid06 full-model fine-tune with AdamW low-lr long-epoch.

Reference previous NWPU best-practice hyperparams (round2129 style):
  AdamW lr=7e-5, weight_decay=1e-4, batch_size=2, epochs=10.
"""
import sys, json
...
DEV = "cuda"; BATCH = 2; CKPT_PATH = "runs/nwpu_baseline_best.pth"
DATA = Path("data/NWPU VHR-10 dataset"); ANNOT = Path("data/NWPU_VHR10_coco.json")
MAX_SIZE = 480; EPOCHS = 10; SEEDS = [42, 123, 456]
...
class NWPUDataset(Dataset):
    ...
    def __getitem__(self, idx):
        info = self.img_infos[self.img_ids[idx]]; img_id = info["id"]
        ...
        tgt = {"boxes": torch.tensor(boxes, dtype=torch.float32), "labels": torch.tensor(labels, dtype=torch.int64), "image_id": torch.tensor([img_id])}
        _, H, W = img.shape
        if max(H, W) > self.max_size:
            scale = self.max_size / max(H, W); nh, nw = int(H*scale), int(W*scale)
            img = nn.functional.interpolate(img.unsqueeze(0), size=(nh, nw), mode="bilinear", align_corners=False).squeeze(0)
...
for seed in SEEDS:
    ...
    best_ap50 = -1; best_ap75 = -1
    for ep in range(1, EPOCHS+1):
        model.train(); total_loss = 0
        for imgs, tgts in tqdm(train_loader, desc=f"mid06_adamw s{seed} e{ep}"):
        ...
        if em['ap50'] > best_ap50:
            best_ap50 = em['ap50']; best_ap75 = em['ap75']
            torch.save({"model": model.state_dict(), "ap50": best_ap50, "ap75": best_ap75},
                       f"runs/nwpu_mid06_adamw_s{seed}_best.pth")
    print(f"  Best: AP50={best_ap50:.4f} AP75={best_ap75:.4f}")
