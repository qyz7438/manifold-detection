"""Compatibility wrapper: implementation moved to ``scripts/analysis/analyze_dense_local_delta_families.py``.
...
- importing ``scripts.analyze_dense_local_delta_families`` re-exports the moved module's public names;
- running ``python scripts/analyze_dense_local_delta_families.py ...`` delegates to the moved module with
  the same arguments, preserving its historical ``__file__`` (the moved module
  resolves repository paths from it).

Update call sites to ``scripts/analysis/analyze_dense_local_delta_families.py``; this wrapper will be
...
_DESTINATION = _OLD_PATH.parent / "analysis" / "analyze_dense_local_delta_families.py"
...
if __name__ == "__main__":
    print(
        "deprecated: scripts/analyze_dense_local_delta_families.py moved to scripts/analysis/analyze_dense_local_delta_families.py; "
        "delegating with identical arguments",
        file=_sys.stderr,
    )
    _load_moved_module("__main__")
else:
    _moved = _load_moved_module("scripts.analysis.analyze_dense_local_delta_families")
    globals().update(
