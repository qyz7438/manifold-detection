"""Compatibility wrapper: implementation moved to ``scripts/analysis/analyze_dense_local_delta_stats.py``.
...
- importing ``scripts.analyze_dense_local_delta_stats`` re-exports the moved module's public names;
- running ``python scripts/analyze_dense_local_delta_stats.py ...`` delegates to the moved module with
  the same arguments, preserving its historical ``__file__`` (the moved module
  resolves repository paths from it).

Update call sites to ``scripts/analysis/analyze_dense_local_delta_stats.py``; this wrapper will be
...
_DESTINATION = _OLD_PATH.parent / "analysis" / "analyze_dense_local_delta_stats.py"
...
if __name__ == "__main__":
    print(
        "deprecated: scripts/analyze_dense_local_delta_stats.py moved to scripts/analysis/analyze_dense_local_delta_stats.py; "
        "delegating with identical arguments",
        file=_sys.stderr,
    )
    _load_moved_module("__main__")
else:
    _moved = _load_moved_module("scripts.analysis.analyze_dense_local_delta_stats")
    globals().update(
