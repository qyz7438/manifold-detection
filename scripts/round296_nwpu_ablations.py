"""NWPU VHR-10 AFM ablations: phase_only, afm_only_lr1e3, afm_only_lr1e4.

All start from nwpu_baseline_best.pth, insert AFM, load non-AFM weights, then fine-tune.
"""
import sys, json
...
RECIPES = [
    ("phase_only_5ep", "mplseg_phase_only", 1e-3, False),
    ("afm_only_lr1e3_5ep", "mplseg_mid", 1e-3, False),
    ("afm_only_lr1e4_5ep", "mplseg_mid", 1e-4, False),
]
...
for seed in SEEDS:
    ...
    for recipe, afm_type, lr, use_constraint in RECIPES:
        ...
        if hasattr(model.roi_heads.box_head.afm, "gate_strength"):
            model.roi_heads.box_head.afm.gate_strength = 0.6
        if hasattr(model.roi_heads.box_head.afm, "residual_scale"):
            model.roi_heads.box_head.afm.residual_scale.data.fill_(1.0)

        # afm_only: freeze all except AFM
        ...
        for ep in range(1, EPOCHS + 1):
            ...
            for imgs, tgts in tqdm(train_loader, desc=f"{recipe} s{seed} e{ep}"):
            ...
            if em['ap50'] > best_ap50:
                ...
                torch.save({"model": model.state_dict(), "ap50": best_ap50, "ap75": em['ap75']},
                           f"runs/nwpu_{recipe}_s{seed}_best.pth")
        print(f"  {recipe} Best: AP50={best_ap50:.4f}")
        if h1: h1.remove()
        if h2: h2.remove()
