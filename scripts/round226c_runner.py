"""Plan 2.26c: Lower-LR continuation from C_5ep to test drift vs LR.

Start from round226_C_5ep_s42 and continue with smaller learning rates.
"""
...
RUNS = [
    ("C_5ep_lr1e4_plus10", 10, 1e-4),
    ("C_5ep_lr5e5_plus10", 10, 5e-5),
]
...
def train_one(seed, extra_epochs, lr, run_name):
    set_seed(seed)
    ...
    for epoch in range(1, extra_epochs + 1):
        ...
        m.update({"run_name": run_name, "extra_epochs": extra_epochs, "lr": lr,
    ...
    final.update({"run_name": run_name, "extra_epochs": extra_epochs, "lr": lr,
...
def main():
    all_r = []
    for name, extra_epochs, lr in RUNS:
        print(f"\n-- {name} (+{extra_epochs} epochs, lr={lr}) --")
        run_name = f"round226c_{name}_s{SEED}"
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                r = train_one(SEED, extra_epochs, lr, run_name)
                all_r.append(r)
    ...
    lines = ["## Plan 2.26c Lower-LR Continuation", "",
             "| Run | LR | AP50 | AP75 | Prec | ECE |",
             "|---:|---:|---:|---:|---:|---:|"]
    for r in all_r:
        lines.append(f"| {r['run_name']} | {r['lr']:.0e} | {r['ap50']:.4f} | {r['ap75']:.4f} | {r['precision']:.4f} | {r['ece']:.4f} |")
    msg = "\n".join(lines)
    print(f"\n{msg}")
