"""Plan 2.76: GRPO + KL Anchor — minimal decision experiment.

4 groups × 3 seeds = 12 experiments:
  A  det_only            supervised fine-tune baseline
  B  det_only_kl         fine-tune + KL anchor to baseline checkpoint
  C  grpo_adv            GRPO group-relative advantage (no verifier, IoU reward)
  D  grpo_fft_loc        GRPO + FFT loc-only residual (IoU >= 0.5 only)

Key changes vs 2.75:
  - GRPO group advantage: (r - mean_group) / std_group per proposal
  - KL anchor: L = L_det + RL_WEIGHT * L_rl + KL_WEIGHT * ||pred - baseline||^2
  - Baseline model frozen copy for KL computation
"""
import copy
import json
import math
...
def grpo_advantage(reward, N, G):
    """GRPO group-relative advantage per proposal: (r - mean_group) / std_group."""
...
def run_one(cfg_name, mode, seed):
    """mode: 'det_only' | 'det_kl' | 'grpo_adv' | 'grpo_fft'"""
    run_name = f"round276_{cfg_name}_s{seed}"
    set_seed(seed)
    model = bm().to(DEV)
    ...
    # KL baseline: frozen copy of the loaded model
    use_kl = mode in ("det_kl",) or mode.startswith("grpo")
    baseline_model = None
    if use_kl:
    ...
    is_det = mode in ("det_only", "det_kl")
    ...
    h = []; best_ap75 = -1.0
    diag = {"q_ious": [], "total_grad_norm": [], "reward_std": [], "kl_loss": [], "norm_mad": []}

    # KL baseline bbox_pred (frozen)
    baseline_bbox_weight = None
    baseline_bbox_bias = None
    if use_kl:
        baseline_bbox_weight = baseline_model.roi_heads.box_predictor.bbox_pred.weight.detach().clone()
        baseline_bbox_bias = baseline_model.roi_heads.box_predictor.bbox_pred.bias.detach().clone()
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        for imgs, tgts in tqdm(tl, desc=f"{run_name} e{ep}"):
            ...
            if not is_det and rf is not None and sp_raw is not None and rf.shape[0] > 0 and fpn is not None:
                ...
                cls_probs = F.softmax(model.roi_heads.box_predictor.cls_score(bf), dim=1)[:, 1]

                # KL: compare bbox_pred weight to baseline
                if use_kl:
                    curr_w = model.roi_heads.box_predictor.bbox_pred.weight
                    curr_b = model.roi_heads.box_predictor.bbox_pred.bias
                    kl_loss = KL_WEIGHT * ((curr_w - baseline_bbox_weight).pow(2).sum() + (curr_b - baseline_bbox_bias).pow(2).sum())
                ...
                if needs_verifier:
                    ...
                    geo = build_geo_features(decoded_cat, image_shapes, img_map)
                    ...
                    # FFT loc-only: only IoU >= 0.5 proposals
                    ...
                    diag["norm_mad"].append(0.0)

                # GRPO group advantage
                adv = grpo_advantage(reward_img, offset, G_SAMPLES)
            ...
            # Backward
            if is_det:
                loss = det + (kl_loss if use_kl else torch.tensor(0.0, device=DEV))
        ...
        kl_avg = np.mean(diag["kl_loss"]) if len(diag["kl_loss"]) > 0 else 0.0
        ...
        row = {"epoch": ep, "val_ap50": em["ap50"], "val_ap75": em["ap75"],
               ...
               "kl_loss": float(kl_avg), "det_loss": td, "rl_loss": trl, "vloss": tv, "pos_count": int(pos)}
        h.append(row)
        print(f"  e{ep}: AP75={em['ap75']:.4f} AP50={em['ap50']:.4f} "
              f"q_corr={q_corr:.4f} tgn={tgn:.6f} r_std={rs_m:.4f} KL={kl_avg:.4f}")
...
if __name__ == "__main__":
    all_results = []
    configs = {"det_only": "det_only", "det_kl": "det_kl", "grpo_adv": "grpo_adv", "grpo_fft": "grpo_fft"}
    for cfg, mode in configs.items():
        for s in SEEDS:
            r = run_one(cfg, mode, s)
            all_results.append(r)

    print("\n## Plan 2.76 GRPO + KL Anchor")
    print(f"  {'Config':<12s} {'Seed':>5s} {'AP75':>8s} {'BestAP75':>8s} {'AP50':>8s} {'Rec':>8s} {'Prec':>8s} {'q_corr':>8s} {'r_std':>8s}")
    for r in all_results:
        best_h = max(r["history"], key=lambda x: x["val_ap75"])
        print(f"  {r['config']:<12s} {r['seed']:5d} {r['ap75']:8.4f} {r['best_ap75']:8.4f} {best_h['val_ap50']:8.4f} {best_h.get('recall', 0):8.4f} {best_h.get('precision', 0):8.4f} {best_h.get('q_iou_corr', 0):8.4f} {best_h.get('reward_std', 0):8.4f}")
    ...
    for cfg in configs:
        ...
        print(f"  {cfg}: bestAP75={np.mean(bv):.4f}±{np.std(bv):.4f}  AP50={np.mean(ap50s):.4f}  Prec={np.mean(precs):.4f}")
    ...
    print("\n## Decision ladder")
    judge("det_kl", "det_only")
    judge("grpo_adv", "det_only")
