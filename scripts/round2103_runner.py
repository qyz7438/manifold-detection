"""Plan 2.103: RLVR on classifier — geo reward (round300 fixed with off-policy).

Type B fix: off-policy sampling + grad clip + tuned weights.
"""
import copy
import shutil
import subprocess
...
def geo_penalty(boxes, img_w, img_h):
    """Penalize small, narrow, edge-positioned boxes. Higher = more central/well-sized."""
    w = boxes[:, 2] - boxes[:, 0]
    h = boxes[:, 3] - boxes[:, 1]
    area = w * h
...
def run_one(cfg_name, mode, seed):
    run_name = f"round2103_{cfg_name}_s{seed}"
    set_seed(seed)
    model = bm().to(DEV)
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        for imgs, tgts in tqdm(tl, desc=f"{run_name} e{ep}"):
            ...
            if not is_det and rf is not None and sp_raw is not None and rf.shape[0] > 0:
                ...
                with torch.no_grad():
                    sigma_per_prop = 0.05 + 0.2 * (1.0 - baseline_cls_conf)  # (N,)
                ...
                # log_prob on CURRENT model — ∂log_prob/∂cls_logits ≠ 0 (off-policy)
                ...
                # Geo reward: compute from baseline box prediction
                sp_cat = torch.cat(sp_raw, dim=0)
                N = min(cls_logits.shape[0], sp_cat.shape[0])
                with torch.no_grad():
                    reg_out = baseline_model.roi_heads.box_predictor.bbox_pred(baseline_bf[:N])
                    person_deltas = reg_out[:, 2:6]
                    decoded = decode_boxes(sp_cat[:N], person_deltas)
                geo_r = geo_penalty(decoded, image_shapes[0][1], image_shapes[0][0])  # (N,) higher=better

                # Reward: geo score, same for all G samples per proposal
                reward_img = geo_r[:N].unsqueeze(1).expand(N, G_SAMPLES)  # (N, G)
                ...
                diag["mean_iou"].append(0.0)  # geo reward, no IoU tracking
...
if __name__ == "__main__":
    ...
    print("\n## Plan 2.103 RLVR on Classifier — IoU-Calibrated Confidence Reward")
