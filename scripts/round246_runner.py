"""Plan 2.46: Edge Centrality RFT.

Replace pixel_fft_quality with edge_centrality in RFT framework (2.41).
Gradient magnitude weighted by distance to center. No FFT, no GT needed.
"""
...
def main():
    ...
    for alpha in ALPHAS:
        run_name = f"round246_cent_a{alpha}_s42"
        set_seed(42)
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            for images, targets in tqdm(train_loader, desc=f"{run_name} e{epoch}"):
                ...
                if roi_feats is not None and proposals is not None and roi_feats.shape[0] > 0:
                    ...
                    pe = proposals_cat[:N].unsqueeze(1).expand(-1, M_SAMPLES, -1).reshape(N * M_SAMPLES, 4)
                    all_boxes = decode_boxes(pe, all_deltas)

                    npi = [p.shape[0] for p in proposals]
                    ii = torch.cat([torch.full((n,), i, dtype=torch.long) for i, n in enumerate(npi)], dim=0)[:N]

                    patches = []
                    for idx in range(min(N * M_SAMPLES, 256)):
                        pi = min(idx // M_SAMPLES, N - 1)
                    ...
                    if patches:
                        ...
                        q = edge_centrality(pb)
                        Kt = N * M_SAMPLES; qp = torch.zeros(Kt, device=DEV); qp[:len(q)] = q
                        qm = qp.view(N, M_SAMPLES)[:N]
                        best_idx = qm.argmax(dim=1)
        ...
        ep_m.update({"run_name": run_name, "alpha": alpha, "epochs": EPOCHS, "seed": 42,
    ...
    print("\n## Plan 2.46 Edge Centrality Results")
    for r in all_r:
