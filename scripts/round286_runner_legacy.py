...
G_SAMPLES = 4; EPOCHS = 8; SEEDS = [42, 123, 456]
RL_WEIGHT = 0.05; KL_WEIGHT = 0.1; BETA = 0.02
...
def geo_penalty(boxes, img_w, img_h):
    ...
    penalty = torch.stack([torch.abs(cx_n), torch.abs(cy_n), -area_n.clamp(-1, 1), -aspect_n.clamp(-1, 1)], dim=1).mean(dim=1)
...
def run_one(cfg_name, mode, seed):
    run_name = f"round286_{cfg_name}_s{seed}"; set_seed(seed)
    ...
    is_det = mode == "det_only_unf"; is_geo = mode == "geo"; opt = build_opt(model)

    h = []; best_ap75 = -1.0; diag = {"reward_std": []}
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        for imgs, tgts in tqdm(tl, desc=f"{run_name} e{ep}"):
            ...
            if not is_det and rf is not None and sp_raw is not None and rf.shape[0] > 0:
                N = rf.shape[0]; bf = model.roi_heads.box_head(rf); mu = model.roi_heads.box_predictor.bbox_pred(bf)[:, -4:]
                ...
                s = torch.full_like(mu, 0.1); deltas = mu.detach().unsqueeze(1) + s.unsqueeze(1) * torch.randn(N, G_SAMPLES, 4, device=DEV)
                log_probs = glp(deltas, mu, s)
                sp_cat = torch.cat(sp_raw, dim=0); N = min(N, sp_cat.shape[0]); deltas = deltas[:N]; log_probs = log_probs[:N]
                ...
                for i_img, p_img in enumerate(sp_raw):
                    ...
                    img_map.extend([i_img] * (n_a * G_SAMPLES)); offset += n_a

                sp_exp = torch.cat([p.repeat_interleave(G_SAMPLES, dim=0) for p in box_list], dim=0)
                delta_cat = torch.cat(delta_list, dim=0)
                bw = sp_exp[:,2]-sp_exp[:,0]; bh = sp_exp[:,3]-sp_exp[:,1]; bcx = sp_exp[:,0]+0.5*bw; bcy = sp_exp[:,1]+0.5*bh
                dx = delta_cat[:,0]/10.0; dy = delta_cat[:,1]/10.0; dw = delta_cat[:,2]/5.0; dh = delta_cat[:,3]/5.0
                ...
                for pi in range(offset):
                    ...
                    if len(gt) > 0: iou_r[pi] = box_iou(decoded_cat[pi*G_SAMPLES:(pi+1)*G_SAMPLES], gt).max(dim=1).values
                ...
                if is_geo:
                    ...
                    # Per-group z-score
                    gp_mean = geo_pen.mean(dim=1, keepdim=True); gp_std = geo_pen.std(dim=1, keepdim=True).clamp_min(1e-6)
        ...
        em = ev(model, vl); rs_m = np.mean(diag["reward_std"]) if diag["reward_std"] else 0
        row = {"epoch": ep, "val_ap50": em["ap50"], "val_ap75": em["ap75"], "pre": em.get("precision",0), "rec": em.get("recall",0), "ece": em.get("ece",0), "reward_std": float(rs_m), "det_loss": td, "rl_loss": trl, "kl_loss": tkl}
        h.append(row); print(f"  e{ep}: AP75={em['ap75']:.4f} AP50={em['ap50']:.4f} r_std={rs_m:.4f}")
