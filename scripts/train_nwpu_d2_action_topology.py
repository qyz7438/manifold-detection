"""Train D2 action-conditioned NMS topology under the locked C3 endpoint."""
...
CONFIG = ROOT / "spectral_detection_posttrain" / "configs" / "versions" / "det.energy.action_topology.d2.001.json"
CONFIG_SHA256 = "2ef50b3152d0aac6349fb0c52e110332420f3419bbf275b4baf829355f3dffe6"
C3_SCRIPT = ROOT / "scripts" / "train_nwpu_c3_global_delta_u.py"
EXPECTED_POLICY = {"architecture": "action_topology"}


def _load_c3():
    spec = importlib.util.spec_from_file_location("d2_c3_source", C3_SCRIPT)
...
def load_config() -> dict[str, Any]:
    if c3.m1.sha256_file(CONFIG) != CONFIG_SHA256:
        raise ValueError("canonical D2 config SHA256 mismatch")
    config = json.loads(CONFIG.read_text(encoding="utf-8"))
    if config.get("version_id") != "det.energy.action_topology.d2.001":
        raise ValueError("D2 version mismatch")
    if config["policy_override"].get("architecture") != EXPECTED_POLICY["architecture"]:
        raise ValueError("D2 action-topology architecture drift")
    if config.get("arms") != ["local_full", "topology_shuffle", "utility_shuffle"]:
        raise ValueError("D2 control arms drift")
    return config


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--run-dir", type=Path, default=ROOT / "runs" / "nwpu_d2_action_topology_smoke_s42")
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if c3.CONFIG_SHA256 != config["source"]["base_config_sha256"]:
        raise RuntimeError("D2 base config hash mismatch")
    source_result = ROOT / config["source"]["d1_result"]
    source_cache = ROOT / config["source"]["cache"]
    if c3.m1.sha256_file(source_result) != config["source"]["d1_result_sha256"]:
        raise RuntimeError("D2 D1 source result hash mismatch")
    if c3.m1.sha256_file(source_cache) != config["source"]["cache_sha256"]:
        raise RuntimeError("D2 global_delta_u_cache.pt hash mismatch")
    ...
    if payload.get("format") != "c3_global_delta_u_v1" or not isinstance(payload.get("records"), list):
        raise RuntimeError("locked D2 cache format mismatch")
    if payload.get("metadata", {}).get("config_sha256") != config["source"]["base_config_sha256"]:
        raise RuntimeError("locked D2 cache provenance mismatch")
    ...
    topology_shuffle_seed = int(effective["controls"]["topology_shuffle_seed"])
    ...
    gates = {
        ...
        "control_delta": all(learned["ap75"] > evaluation["metrics"][arm]["ap75"] for arm in ("topology_shuffle", "utility_shuffle")),
    }
    all_passed = all(gates.values())
    d1 = json.loads(source_result.read_text(encoding="utf-8"))
    result = {
        "completed": True,
        "scientific_status": "larger_train_confirmation_allowed" if all_passed else "d2_action_topology_frozen_move_d3",
        "version_id": config["version_id"],
        "experiment_scope": "action_topology_smoke_32_32",
        "config_sha256": CONFIG_SHA256,
        "base_config_sha256": config["source"]["base_config_sha256"],
        "source_cache_sha256": config["source"]["cache_sha256"],
        "source_d1_result_sha256": config["source"]["d1_result_sha256"],
        "topology_shuffle_seed": topology_shuffle_seed,
        "source_d1_summary": d1.get("summary"),
