"""Compatibility wrapper: implementation moved to ``scripts/analysis/audit_dense_endpoint_attribution.py``.
...
- importing ``scripts.audit_dense_endpoint_attribution`` re-exports the moved module's public names;
- running ``python scripts/audit_dense_endpoint_attribution.py ...`` delegates to the moved module with
  the same arguments, preserving its historical ``__file__`` (the moved module
  resolves repository paths from it).

Update call sites to ``scripts/analysis/audit_dense_endpoint_attribution.py``; this wrapper will be
...
_DESTINATION = _OLD_PATH.parent / "analysis" / "audit_dense_endpoint_attribution.py"
...
if __name__ == "__main__":
    print(
        "deprecated: scripts/audit_dense_endpoint_attribution.py moved to scripts/analysis/audit_dense_endpoint_attribution.py; "
        "delegating with identical arguments",
        file=_sys.stderr,
    )
    _load_moved_module("__main__")
else:
    _moved = _load_moved_module("scripts.analysis.audit_dense_endpoint_attribution")
    globals().update(
