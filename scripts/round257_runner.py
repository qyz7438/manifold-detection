"""Plan 2.57: DPO with native-resolution pixel FFT quality.

Key fix over 2.55: NO resize to 64x64. Crops keep native resolution, zero-padded
to 64x64. This preserves original pixel boundaries — 1-2px shift produces
measurable content change (~2-5% at native res vs <1% after bilinear resize).

Zero-padding = sinc interpolation in frequency domain = original frequencies
preserved, just sampled more finely. Bilinear resize = low-pass filter = loses
the high-frequency boundary information we need to detect small shifts.
"""
...
def native_pixel_fft_quality(patches):
    """2.31 formula on NATIVE-res zero-padded patches (not resized).

    patches: list of tensors with different native sizes (3, H_i, W_i)
    Each is zero-padded to (3, 64, 64) preserving original content.
    """
    padded = []
    for p in patches:
        h, w = p.shape[-2], p.shape[-1]
        if h < 4 or w < 4:
            # Degenerate crop: use zero patch
            padded.append(torch.zeros(3, PAD_SIZE, PAD_SIZE, device=p.device))
        elif h >= PAD_SIZE or w >= PAD_SIZE:
            padded.append(F.interpolate(p.unsqueeze(0).float(), (PAD_SIZE, PAD_SIZE), mode='area').squeeze(0))
        else:
            pp = torch.zeros(3, PAD_SIZE, PAD_SIZE, device=p.device)
            pp[:, :h, :w] = p.float()
            padded.append(pp)
    pb = torch.stack(padded)

    # Same quality formula as 2.31
    gray = pb.mean(dim=1)
    ...
    max_e = torch.log(torch.tensor(float(mag_flat.shape[1]), device=pb.device))
...
def main():
    ...
    for beta in BETAS:
        run_name = f"round257_dpo_native_b{beta}_s42"
        set_seed(42)
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            for images, targets in tqdm(train_loader, desc=f"{run_name} e{epoch}"):
                ...
                if roi_feats is not None and proposals is not None and roi_feats.shape[0] > 0:
                    ...
                    # Decode + crop NATIVE patches (NO resize, zero-pad)
                    ...
                    Kt = N * K_SAMPLES
                    if raw_patches:
                        q_all = native_pixel_fft_quality(raw_patches)
                    ...
                    dpo_loss = -F.logsigmoid(beta * ratio).mean()
    ...
    print("\n## Plan 2.57 DPO-Native (Zero-Pad, No Resize) Results")
    for r in all_r:
