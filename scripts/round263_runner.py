"""Plan 2.63 v2: IoU + ROI + FFT verifier GRPO, per-sampled-box.

FIXES: per-box ROI Align, vloss trains verifier (no detach on q_pred), IoU before vloss.
"""
...
DEV="cuda";CKPT="runs/round227_v1_baseline_20ep/checkpoint_best.pth";G=4;EPOCHS=5;ROI_SIZE=14  # 14x14 FFT: 112 bins vs 28 for 7x7

class FFTVerifier(nn.Module):
    def __init__(self,roi_dim,fft_dim,hidden=128):
        super().__init__()
        self.net=nn.Sequential(nn.Linear(roi_dim+fft_dim,hidden),nn.ReLU(),nn.Linear(hidden,32),nn.ReLU(),nn.Linear(32,1),nn.Sigmoid())
    def forward(self,r,fft_f):return self.net(torch.cat([r.flatten(1),fft_f],dim=1)).squeeze(-1)
...
def main():
    all_r=[]
    for lbd in [0.1,0.5]:
        run_name=f"round263_fft14_l{lbd}_s42";set_seed(42)
        ...
        tl,vl=bl();vrf=None;rd=ensure_run_dir(run_name);h=[];best=-1.0
        pc={};rc={};fc={}
            ...
            for imgs,tgts in tqdm(tl,desc=f"{run_name} e{ep}"):
                ...
                if rf is not None and pr is not None and rf.shape[0]>0 and fpn is not None:
                    ...
                    for ki,k in enumerate(fpn_keys):
                        ...
                        if vrf is None:
                            vrf=FFTVerifier(r.flatten(1).shape[1],fft_f.shape[1]).to(DEV)
    ...
    print("\n## Plan 2.63 v2 Per-Box FFT Verifier Results")
