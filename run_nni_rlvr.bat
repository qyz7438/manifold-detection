...
%PYTHON% -c "import nni"
if errorlevel 1 (
  echo NNI is not installed. Install with: %PYTHON% -m pip install nni==3.0
  pause
  exit /b 1
)

%NNICTL% create --config nni_configs\rlvr_config.yml --port 8080
if errorlevel 1 pause & exit /b 1
echo RLVR NNI experiment started at http://localhost:8080
pause
